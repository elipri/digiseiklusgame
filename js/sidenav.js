document.addEventListener("DOMContentLoaded", (event) => {
  ...
  if(d) {
    d.addEventListener("click", ()=> {
      ...
      let pdf = new jsPDF({filters: ["ASCIIHexEncode"]});
  ...
  let results = document.getElementById("responsecontainer");
  let resultlist = document.getElementsByClassName("results");
  if (results) {
    setInterval(resultChange, 1000);
    function resultChange(){
      console.log('resultChange running');
